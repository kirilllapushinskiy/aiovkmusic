...
RPS_DELAY_RELOAD_AUDIO = 0
RPS_DELAY_LOAD_SECTION = 0
...
class VkAudio(object):
    ...
    def search_iter(self, q, offset=0):
        ...
        loguru.logger.info(
            f"From query '{q}' section response {response.status_code}: {response.headers['Content-Type']}"
        )
        ...
        while True:
            ...
            loguru.logger.info(
                f"From query '{q}' load_catalog_section response {response.status_code}: {response.headers['Content-Type']}"
            )
            json_response = json.loads(response.text.replace('<!--', ''))
